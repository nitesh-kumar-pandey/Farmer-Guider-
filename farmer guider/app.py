...
import pandas as pd
import os
from farmerdata import create_table, insert_form_data  # Importing from farmerdata.py

# Load the KMeans model, scaler, and data
try:
    kmeans = joblib.load(os.path.join("models", "kmeans_model.lb"))
    std = joblib.load(os.path.join("models", "standardscaler.lb"))
    df = pd.read_csv(os.path.join("models", "filtering_data.csv"))
except Exception as e:
    raise Exception(f"Error loading models or data: {e}")

# Define the item images and names
item_images = {
...
def submit():
    try:
        ...
        insert_form_data(n, p, k, temperature, humidity, ph, rainfall)
        ...
        # Retrieve the crops corresponding to the predicted cluster
